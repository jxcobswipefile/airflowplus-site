  ...
  ready(function(){
    try {
      ...
      function getGap(){
        ...
        return isFinite(gap) && gap >= 0 ? gap : 16;
      }
      ...
      function ensureImagesLoaded(cb){
        ...
        if (left === 0) return cb();
        function check(){ if (!done && --left <= 0){ done = true; cb(); } }
      ...
      function scrollToRatio(r){
        track.scrollLeft = maxScroll() * r;
      }

      // Use viewport "page" width for navigation to prevent early wrap
      function pageWidth(){
        // move by ~1 viewport minus a small epsilon so images settle nicely
        return Math.max(120, track.clientWidth * 0.92);
      }
      ...
      if (window && window.console) console.warn('Gallery init skipped:', e);
    }
  });
})();
